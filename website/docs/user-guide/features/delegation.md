...
One exception: when the parent has a resolved workspace directory, every subagent's system prompt embeds the parent's profile-scoped **configured global instruction files**, followed by that workspace's **project context files** (`.hermes.md` > AGENTS.md chain > CLAUDE.md > `.cursorrules` — the same discovery, priority, deduplication, and size caps as the main agent's system prompt; SOUL.md is excluded). Subagents working in a repo operate under fleet and repo conventions without having to rediscover them. The configured list is copied from the parent agent's frozen profile snapshot, not re-read from ambient config.
...
3. The reviewer inherits the primary agent's working context: any skills the primary agent had loaded (launch-preloaded or via `skill_view` during the session) are named in its briefing with an instruction to load them and judge the work against their conventions. Like every subagent, its system prompt also embeds configured global instructions followed by the workspace's project context files (AGENTS.md / CLAUDE.md / .cursorrules) as binding conventions.
