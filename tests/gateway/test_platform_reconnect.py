...
class TestStartupPlatformIsolation:
    ...
    @pytest.mark.asyncio
    async def test_routing_only_multiplex_still_starts_primary_adapter(
        ...
        runner._send_update_notification = AsyncMock(return_value=True)
        runner._send_restart_notification = AsyncMock()
        primary = StubAdapter(platform=Platform.TELEGRAM)
        runner._create_adapter = MagicMock(return_value=primary)
        runner._connect_adapter_with_timeout = AsyncMock(return_value=True)

        monkeypatch.setattr(
            "hermes_cli.profiles.profiles_to_serve", lambda multiplex: []
        )
        ...
        assert runner.adapters == {Platform.TELEGRAM: primary}
